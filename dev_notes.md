...
- Open `bar-tracker.html` in your browser. On load, the page now checks `http://localhost:4100/server/status` and, if needed, calls `POST /server/start` to launch `python3 -m http.server 8000` for you.
...
- To tweak ports without editing code, set `BAR_TRACKER_CONTROL_ORIGIN` or `BAR_TRACKER_APP_PORT` on `window` before the React bundle runs (e.g., via a small inline `<script>`).
